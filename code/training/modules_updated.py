...
def split_data(X_data, y_data,log_path='output/output.txt'):
    output_file = open(log_path, 'a')
...
def evaluate_model(y_true, y_pred, log_path='output/output.txt'):
    output_file = open(log_path, 'a')
    ...
    print(classification_report(y_true, y_pred), file=output_file)
    output_file.close()
